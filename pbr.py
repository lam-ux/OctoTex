...
def generatePBR():
	...
	for x in tqdm( os.listdir(f"textures/processing/diffuse/"), desc="Generating..." ):
		already = hasherObj.materialized(f"textures/processing/diffuse/{x}")
		if( already and not force ):
			continue

		if x.endswith(".png"):
			generate_normal(
				f"textures/processing/diffuse/{x}",
			...
			hasherObj.add_materialized(f"textures/processing/diffuse/{x}")
